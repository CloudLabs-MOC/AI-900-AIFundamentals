...
1. In the **Terms of Service** box, check the box **(1)** and click on **I agree (2)**.
     ...
     ![Start Cloud Shell by clicking on the icon to the right of the top search box](../media/gt18.png)

1. Click **NEW PROJECT**.

   ![Start Cloud Shell by clicking on the icon to the right of the top search box](../media/gt19.png)
...
1. Create a new project with the following settings:
    ...
    - Click on **Create Project (6)**
      ...
      ![Start Cloud Shell by clicking on the icon to the right of the top search box](../media/gt20.png)
...
1. Right click on the zip file **(1)** and select **Extract all (2)** to extract the training images. 
...
1. Hold the mouse over any object (cyclist or pedestrian) in the image until an automatically detected region is displayed **(1)**. Then select the object, and if necessary resize the region to surround it. Alternatively, you can simply drag around the object to create a region.
    ...
    When the object is tightly selected within the rectangular region, enter the appropriate tag for the object (**Cyclist** or **Pedestrian**) **(2)** and use the **Tag region** (**+**) button to add the tag to the project **(3)**.
...
1. When you have finished tagging the last image, close the **Image Detail** editor, and on the **Training Images** page, under **Tags**, select **Tagged (1)** to see all of your tagged images:
...
1. Select the **Quick Training (1)** option, Again click **Train (2)**.
    ...
    ![Start Cloud Shell by clicking on the icon to the right of the top search box](../media/gt32.png)
...
3. Adjust the **Probability Threshold** on the left, increasing it from `50% to 90%` **(1)**, and observe the effect on the performance metrics. This setting determines the probability value that each tag evaluation must meet or exceed to be counted as a prediction **(2)**.

      ![Screenshot of performance metrics for a trained model.](../media/gt34.png)

4. At the top right of the page, click **Quick Test**.

      ![Screenshot of performance metrics for a trained model.](../media/quicktest-3b.png)

5. Then in the **Image URL** box, enter `https://aka.ms/pedestrian-cyclist` **(1)**, click on the send icon **(2)** and view the results.      
...
1. To publish the trained model with the following settings:
    ...
    - Click **Publish (3)**
      ...
      ![Start Cloud Shell by clicking on the icon to the right of the top search box](../media/gt36.png)
...
1. If the Code editor is closed, then only run the below commnd. The files are downloaded to a folder named **ai-search**. Now we want to see all of the files in your Cloud Shell storage and work with them. Type the following command into the shell:
...
1. Don't worry too much about the details of the code. The important thing is that it starts with some code to specify the prediction URL and key for your Custom Vision model. You'll need to update these so that the rest of the code uses your model.
    ...
    ![Start Cloud Shell by clicking on the icon to the right of the top search box](../media/gt39.png)
    ...
    ![Start Cloud Shell by clicking on the icon to the right of the top search box](../media/gt41.png)    
